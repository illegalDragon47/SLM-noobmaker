...
class DeepSeekRMSNorm(nn.Module):
    """DeepSeek-style RMS Normalization with optional bias."""
...
class DeepSeekSwiGLU(nn.Module):
    """DeepSeek-style SwiGLU activation with specific gating mechanism."""
...
class DeepSeekAttention(nn.Module):
    """DeepSeek-style attention with specific optimizations and scaling."""
...
class DeepSeekBlock(nn.Module):
    """DeepSeek transformer block with specific architecture choices."""
    
    def __init__(self, config: ArchitectureConfig):
        super().__init__()
        self.hidden_size = config.n_embd
        self.self_attn = DeepSeekAttention(config)
        self.mlp = DeepSeekSwiGLU(
            self.hidden_size,
            int(4 * self.hidden_size * 2 / 3),  # DeepSeek uses 2/3 ratio
            bias=config.bias,
        )
        self.input_layernorm = DeepSeekRMSNorm(self.hidden_size, eps=1e-5)
        self.post_attention_layernorm = DeepSeekRMSNorm(self.hidden_size, eps=1e-5)
...
class DeepSeekStyleArchitecture(BaseArchitecture):
    """DeepSeek-style transformer architecture with authentic components."""
    ...
    def __init__(self, config: ArchitectureConfig):
        ...
        self.norm = DeepSeekRMSNorm(config.n_embd, eps=1e-5)
        
        # Initialize weights
        self.apply(self._init_weights)
        
    def _init_weights(self, module: nn.Module):
        """Initialize weights with DeepSeek-specific scaling."""
